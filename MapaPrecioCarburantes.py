...
for i in range(len(dfprov)):
    folium.CircleMarker(location=[dfprov.Latitud.iat[i],dfprov.Longitud.iat[i],],popup=dfprov.data.iat[i],radius=5,color=dfprov.color.iat[i],fill=True, fill_opacity=0.7).add_to(m)
